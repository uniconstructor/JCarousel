...
class JCarousel extends CBaseListView
{
    ...
    public $text;
    /*
     * text,title or description below img
     */

    public $catId;
    /*
     * id image for active status
     */
	...
	private function renderGalleryItems()
	{
		foreach ($this->dataProvider->getData() as $data) {
			...
			if (isset($this->target))
				echo $li.'<a href="'.eval('return '.$this->imageUrl.';').'" class="'.eval('return '.$this->linkClass.';').'">'.$image.'</a>'.$text.'</li>';
			else
				echo $li.$image.$text.'</li>';
